...
const MobileNav = ({
  ...
  return (
    <nav class='dark:(bg-slate-800, text-pink-600) absolute top-0 min-w-full select-none bg-slate-100 p-3 text-2xl text-violet-800 transition-colors lg:hidden'>
      <button
        onClick={(): void => {
          setDrawerOpen(true);
        }}
...
const Sidebar = ({
  ...
  return (
    <nav
      ...
      } dark:(bg-slate-700, text-pink-600) w-72 select-none flex-col items-start overflow-y-auto bg-slate-200 p-3 text-2xl text-violet-800 lg:flex xl:w-80 2xl:w-96`}>
      <span class='flex min-w-full gap-2'>
        <button class='hover:(bg-slate-100, active:bg-slate-50, dark:(bg-slate-800, active:bg-slate-900)) flex grow items-center gap-3 break-all rounded-lg p-3 font-medium leading-4 transition-colors'>
      ...
      <span class='mt-2 flex min-w-full flex-col gap-2' />
    </nav>
  );
};
