    ...
    <div class='flex' ref={refer}>
      {sortMode ?
        <button
          ...
          title='Toggle Order'
          id='toggle-order'
          class='hover:(bg-slate-200, active:bg-slate-100, dark:(bg-slate-700, active:bg-slate-800)) md:(max-w-[8rem], text-base, gap-1.5, pl-3.5, pr-3, py-2) flex max-w-[7rem] items-center justify-end gap-1 rounded-l py-1 pr-2 pl-2.5 text-sm font-medium whitespace-nowrap duration-150'
          onClick={(): void => {
      ...
      <button
        ...
        } hover:(bg-slate-200, active:bg-slate-100, dark:(bg-slate-700, active:bg-slate-800)) md:(max-w-[8rem], text-base, gap-1.5, px-2.5, py-2) flex max-w-[7rem] items-center justify-end gap-1 rounded-r px-1.5 py-1 text-sm font-medium whitespace-nowrap duration-150`}
        onClick={(): void => {
      ...
      {isVisible && (
        <div
          id='sort-dropdown'
          class='dark:(bg-slate-700, shadow-slate-600) absolute z-10 mt-[2.35rem] -ml-[1.4rem] flex flex-col items-end justify-center overflow-clip rounded bg-slate-200 shadow-sm shadow-slate-300 md:mt-12'>
          {Object.keys(SORT_OPTIONS).map(option => (
            <button
              type='button'
              key={option}
              class='hover:(bg-slate-300 dark:(bg-slate-600 active:bg-slate-500)) flex w-24 min-w-fit justify-end px-2 py-[3px] font-medium select-none active:bg-slate-400 md:w-28'
